...
def SalUn(data_loaders, model, criterion, optimizer, epoch, args):
    ...
    if args.imagenet_arch:
        for i, data in enumerate(forget_loader):
            ...
            gradient_mean_forget, gradient_variance_variance = vutils.get_grad_mean_var(model, criterion, image, target)

            p=0.8
            phi_forget = torch.distributions.Normal(0, 1).cdf(torch.abs(gradient_mean_forget) / var)
