...
@iterative_unlearn
def MSG(data_loaders, model, criterion, optimizer, epoch, args):
    ...
    if epoch == 0:
        min_aggregation = [0. for param in model.parameters()]
        ...
        for i, (image, target) in enumerate(forget_loader):
            ...
            for idx_param, param in enumerate(model.parameters()):
                min_aggregation[idx_param] = min_aggregation[idx_param] + torch.min(torch.abs(param.grad), torch.abs(grads[idx_param]))
        
        with torch.no_grad():
            for idx_param, param in enumerate(model.parameters()):
                mask = (min_aggregation[idx_param] <= torch.quantile(min_aggregation[idx_param], 0.3, interpolation='midpoint'))
                noise = torch.randn_like(param) * param
