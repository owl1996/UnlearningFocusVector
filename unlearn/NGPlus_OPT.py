...
@iterative_unlearn
def NGPlus_OPT(data_loaders, model, criterion, optimizer, epoch, args, VF = None, VR = None):
    ...
    for i, (image, target) in enumerate(forget_loader):
        ...
        for idx_param, param in enumerate(model.parameters()):
            ...
            grad_u = grad_forget[idx_param].view(-1)
            grad_2u = grad_u * grad_u

            grad_c = param.grad.view(-1)
            ...
            param.grad = mask_most_neg * (args.beta * param.grad + (1 - args.beta) * grad_forget[idx_param])
